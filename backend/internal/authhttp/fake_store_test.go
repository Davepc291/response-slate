...
// A minimal in-memory fake of identityservice.Store/AuditRecorder, used
// only to exercise this package's HTTP transport behavior without a live
// database. It intentionally mirrors identityservice's own test fake
// (backend/internal/identityservice/fake_store_test.go): the two packages
// test different layers (business logic vs. HTTP wiring) and each needs
// its own lightweight double rather than sharing a test-only type across a
// package boundary Go has no lightweight mechanism for.
...
type fakeSession struct{ session.Session }
